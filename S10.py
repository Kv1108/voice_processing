...
# Function to save clustered audio segments
...
# Function to estimate the number of speakers based on features
...
# Main function to process VAD and apply clustering
def process_vad_and_clustering(audio_file):
    ...
    # Extract MFCC features from the audio segments
    mfcc_features = []
    ...
    # Estimate the number of clusters (speakers)
    ...
    # Apply UMAP for dimensionality reduction (visualization)
    if len(mfcc_features_scaled) > 1:
        umap_model = umap.UMAP(n_components=2, random_state=42)
        reduced_features = umap_model.fit_transform(mfcc_features_scaled)
