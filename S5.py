...
def estimate_num_speakers(features, max_speakers=5):
    ...
    for n in range(2, min(max_speakers + 1, len(features))):
        gmm = BayesianGaussianMixture(n_components=n, random_state=0, weight_concentration_prior_type='dirichlet_process')
...
def process_vad_and_clustering(audio_file):
    ...
    # Applying Dirichlet Process Gaussian Mixture Model
    gmm = BayesianGaussianMixture(n_components=n_clusters, random_state=0, weight_concentration_prior_type='dirichlet_process')
    labels = gmm.fit_predict(mfcc_features_scaled)
